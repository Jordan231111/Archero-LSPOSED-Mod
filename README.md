...
- **Free Story shield:** Hooks `GameLogic.send_use_key` so Free Story plays never reach the network and the local Key counter stays in lockstep with the server. Optional `LocalSave.Modify_Key` / `LocalSave.UserInfo.SetKey` co-hooks add defense-in-depth resync blocking. See [`docs/free-story-shield.md`](docs/free-story-shield.md).
