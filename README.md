...
- **Free Story shield:** Hooks `GameLogic.send_use_key` and `NetManager.SendInternal<object>` so Free Story battle starts still reach the server, but the scoped `CLifeTransPacket` key cost is zeroed and the matching local predictive Key decrement is blocked. Optional `LocalSave.UserInfo.SetKey` blocking remains available for resync diagnostics. See [`docs/free-story-shield.md`](docs/free-story-shield.md).
